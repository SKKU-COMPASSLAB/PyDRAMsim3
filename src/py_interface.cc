...
void *pydramsim3_create_msys(char *config_file, char *output_dir) {
    return new MemorySystemWrapper(config_file, output_dir);
}
...
void *pydramsim3_create_msys_cmd(msys_data_t addr, msys_data_t size, char is_write) {
    return new MemorySystemCommand{(uint64_t)addr, (uint64_t)size, (bool)is_write, 0, 0, 0};
}
...
char  pydramsim3_msys_dispatch_cmd(void *msys, void *cmd_p) {
    return static_cast<MemorySystemWrapper *>(msys)->dispatch_command(static_cast<MemorySystemCommand *>(cmd_p));
}
